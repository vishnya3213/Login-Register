...
void saveGameHistory(const string& filename, const string& history) {
    ofstream file(filename, ios::app); // запись , Открываем файл в режиме добавления так же не удаляет прошлые данные
    if (file.is_open()) {
        file << history << endl;
        file.close();
    }
...
void loadGameHistory(const string& filename) { // указывает на ссылку filename
    ifstream file(filename); // для чтения
    if (file.is_open()) {
        string line; 
            ...
            while (true) {
                ...
                case 1: { // Монетка
                    ...
                    do {
                        ...
                        bool win = (a == choose); // истина или ложь
                ...
                case 2: { // Рулетка
                    ...
                    do {
                        ...
                        cout << "2 - Чёрный" << endl;
                        cin >> color;
                        if (color != 1 && color != 2) { // если color не равен 1 и 2 то будет ошибка
                        ...
                        bool winNumber = (bet == result); // да или нет
                        bool winColor = (color == resultColor);
                        if (winNumber && winColor) {
                            updateBalance(&balance, 100); //  амперсанд
